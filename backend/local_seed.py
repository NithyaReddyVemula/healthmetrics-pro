"""
Local SQLite seed script (no raw SQL schema prefixes).
Run: venv/Scripts/python local_seed.py
"""
import os
os.environ["DATABASE_URL"] = "sqlite:///./healthmetrics.db"

import random
from datetime import date

from app.models.database import engine, SessionLocal, Base, apply_sqlite_patch
...
def random_date(start_year: int = 2023) -> date:
    ...
    return start + __import__("datetime").timedelta(days=random.randint(0, delta))
...
def generate():
    ...
    try:
        ...
        db.query(QualitySummary).delete()
        db.query(HedisMeasure).delete()
        ...
        db.commit()
        print(f"Done. 500 patients, {500 * len(HEDIS_MEASURES)} measures, {len(HEDIS_MEASURES)} summaries.")
